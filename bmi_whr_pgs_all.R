...
### Uploading bmi and whr data:
bmi <- vroom(file = "Meta-analysis_Locke_et_al+UKBiobank_2018_top_941_from_COJO_analysis_UPDATED.txt")
whr_primary <- vroom(file = "whr_primary.txt")
whr_secondary <- vroom(file = "whr_secondary.txt")
...
bmi_clumped <- clump_data(bmi_renamed,
                  ...
                  clump_p2 = 1)
dim(bmi_clumped)
write.csv(bmi_renamed, file = "bmi_unclumped_pgs(n=941).csv", row.names = FALSE, quote = FALSE)
write.csv(bmi_clumped, file = "bmi_clumped_pgs(n=333).csv", row.names = FALSE, quote = FALSE)
...
whr_selected <- whr %>%
  select(SNP, Chr.ref.males, Pos.ref.males, A1.combined, A2.combined, frqA1.combined, beta.combined, se.combined, pval.combined)
whr_selected <- whr_selected %>%
  add_column(N = 697734)
...
whr_clumped <- clump_data(whr_renamed,
                  ...
                  clump_p2 = 1)
dim(whr_clumped)
write.csv(whr_renamed, file = "whr_unclumped_pgs(n=736).csv", row.names = FALSE, quote = FALSE)
write.csv(whr_clumped, file = "whr_clumped_pgs(n=321).csv", row.names = FALSE, quote = FALSE)